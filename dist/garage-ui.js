  ...
  render() {
    ...
    $("part-grid").innerHTML = visibleParts
      .map((part) => {
        ...
        const upgradeReady = best > tier;
        return `<button class="part-tile ${tier ? "is-upgraded" : "is-stock"} ${upgradeReady ? "has-upgrade" : ""}" data-part="${part.id}" data-inspect="${part.id}" data-tier="${previewTier}" data-installed-tier="${tier}" aria-label="${vehiclePartName(part, this.car)}, ${TIERS[tier].name} currently fitted${stars ? `, ${stars} star boost` : ""}${upgradeReady ? `, ${TIERS[best].name} upgrade ready to fit` : ""}" aria-pressed="${part.id === this.selectedPart}" style="--tier:${TIERS[tier].color}"><span class="tile-photo">${partArtwork(part, "", tier || 1)}${upgradeReady ? '<span class="upgrade-pulse" aria-hidden="true"><b>↑↑</b><small>FIT</small></span>' : ""}</span><span class="tile-copy"><span class="part-title-row"><b>${vehiclePartName(part, this.car)}</b><span class="installed-badge">✓ ${TIERS[tier].name.toUpperCase()}${stars ? ` ${"★".repeat(stars)}` : ""}</span></span><small>${part.effect}</small><span class="tile-tier-meter" aria-hidden="true">${levelBars}</span>${upgradeReady ? `<em class="upgrade-ready">↑ UPGRADE READY · ${TIERS[best].name.toUpperCase()}</em>` : tier ? `<em class="power-level">LEVEL ${tier}/${maxTier}</em>` : '<em class="factory-level">READY TO UPGRADE</em>'}</span></button>`;
      })
      .join("");
