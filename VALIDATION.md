...
- `npm run build` and `npm run check:build` succeed. The generated client contains one versioned media pack with 30 byte-identical files (41,859,437 bytes); no QA fixture or reference-only asset is included. The generic Sites build wrapper encountered a Windows npm-path issue; the established project build completed successfully.
