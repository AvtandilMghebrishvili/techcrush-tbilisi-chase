...
## v2.5.1 - Faster map loading and bounded lifetimes

- All **228 automated tests** pass, including new exact tree-transform/LOD/rewind comparisons, static batch geometry and child-event preservation, selected-car repair/reuse, shared GPU ownership, late-load/decode cancellation, shader polling cancellation and immutable-media scope. Existing three-city roads, collisions, drift, mobile controls, saves, rewards and leaderboards remain covered.
- `npm run build` and `npm run check:build` succeed. The generated client contains one versioned media pack with 30 byte-identical files (41,859,437 bytes); no QA fixture or reference-only asset is included. Because authored source also lives in `dist/`, `node scripts/prepare-release.mjs` stages only `dist/client`, `dist/server` and `dist/.openai` before the Sites packager; the full authoring tree is never the deployment archive. The generic Sites build wrapper encountered a Windows npm-path issue; the established project build completed successfully.
- Local browser checks cover Tbilisi/Kutaisi/Batumi initialization, map selection, garage/drive/pause reuse and permanent teardown with an isolated in-memory database. Map switches leave browser history length unchanged. Resource counts stay bounded in the six-cycle sample; menu probes emit zero frames, and teardown closes both WebGL contexts. No test records are sent to the public game.
- See [performance details](docs/PERFORMANCE.md) for measured Kutaisi/Batumi assembly/load improvements, tree CPU timings and limitations. No save/schema migration, new media, graphics reduction or physical-phone FPS claim.
