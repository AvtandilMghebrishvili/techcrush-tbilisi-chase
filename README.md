...
Run `npm test` for the 19 deterministic mechanics and regression tests. Run `node tests/route-drive.mjs` for a complete route with the same throttle, steering, brake and boost inputs used during play, with traffic and police active. Append `rally` or `suv` to test the other cars. Each run must clear all six checkpoints and escape successfully. The smoke controller is only a test utility and is not part of the playable game. See `VALIDATION.md` for results.
