...
## September 2026: city rendering, rooftop routes and recovery pins

Static architecture now uses 320 m spatial batches with tile-local vertex coordinates. Every triangle, material, UV and original shadow flag is preserved; instanced/dynamic objects remain independent. Repeated façade materials share their generated atlases, and all 160 tyre marks share one plane geometry. Facade/trim depth separation and a tighter outdoor near plane address coastal high-rise flicker. Supported renderers use reversed depth, with matching polygon-offset direction; unsupported browsers keep normal depth. Cockpit and hood close-up near planes remain separate.

A same-camera local Batumi comparison reduced renderer calls from **3,382 to 2,152 (36.4%)**, with uploaded geometries from **3,885 to 2,158**. Textures stayed at **89**; triangles rose from 574,572 to 590,118 because of the new rooftop sites and coarser batching. These are one controlled view's counts, not a promised FPS improvement on every device. In a separate warmed ORBI view, eight restarts each held **2,253 geometries / 91 textures**. Disposing the view returned both counters to **0**. Day/night tower inspection confirmed continuous cornices without competing facade pixels.

Each city has two additional rooftop jumps, bringing its rooftop boxes to three. New yards replace only reserved ordinary lots within existing bounds. Their ramps, launch corridors and roof collision heights are tested with actual driving/landing simulation, including failed low-speed approaches, rewind and one-time rewards. Original roof/gap IDs, published CITY WARS artifact coordinates, course versions, garage saves and standings remain intact. New boxes use shared geometry and the existing GREX texture; no new image downloads or rendering loops are added.

The full map and heading-up minimap show currently available repair gears and dinosaur pulses, independent of the building-banner toggle. Nearby badges separate without losing their exact target coordinates. Red GRA gears restore HP to 100%; green GREX pulses cost 50 HP and clear pursuit temporarily. Clicking a full-map icon sets the secondary yellow waypoint while checkpoints remain active. Used pickups disappear from the map until the next run, and rewind restores them.

The complete automated suite passes **294 tests**, including expanded reward settlement, saved-state compatibility, hidden/idle scheduling, resource disposal, NPC ramps and all four cities' jump trajectories. Browser QA uses a separate local origin and test scene; no test scores or owner overrides are sent to production. Source references, screenshots, test fixtures and tooling stay outside the runtime archive. Existing fingerprinted shared media is reused across maps; no service worker or extra persistent Cache Storage was introduced. Useful browser HTTP caching and saved profiles are retained.
