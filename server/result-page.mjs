...
export async function resultPage(request, DB) {
  ...
  const map = r.course.startsWith("kutaisi-") ? "kutaisi" : "tbilisi";
  const time = formatRaceTime(r.duration_ms),
    title = `${r.display_name} · ${map.toUpperCase()} Level ${r.level} in ${time} · TECHCRUSH`,
    description = `${r.score.toLocaleString("en-US")} points · ${carSpec(r.car).name} · ${r.build_points}/70 upgrades · ${r.rewinds} rewinds. Six checkpoints, one escape. Can you beat this time?`,
